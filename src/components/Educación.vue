<!-- Eslint-distable vue/multi-word-component-names -->
<script setup>
import { ref } from 'vue';
...
<template>
    <ul>
        ...
        <!-- Aquí vemos con el uso de b-vind (:) que bindeamos el atributo href con el item.enlace. -->
...
ul {
    margin-top: 2rem; 
    --col-gap: 2rem; /* Espacio entra las columnas de la cuadrícula. */
    ...
    grid-auto-columns: max-content; /* La columnas se ajustan automáticamente al contenido. */
...
ul li {
    ...
    grid-row: span2; /* Cada ítem ocupa dos filas en la cuadrícula. */
...
ul li .fecha {
    ...
    height: var(--dateH); /* Aplica altura definida. */
    ...
    color: white; /* Color de texto en la fecha. */
    font-size: 1.25rem; /* Tamaño del texto.*/
    font-weight: 700; /* Hce el texto en negrita. */
    display: grid; /*Usa un layout de cudricula. */
...
ul li .fecha::before {
    ...
    background-image: linear-gradient(rgba(0, 0, 0, 0, 0.2) 100%, transparent); /* Aplica un degradado sutil para dar un efecto de sombra. */
    ...
    right: 0; /* Alinea el triánguloa la derecha de la fecha. */
}
...
ul li .fecha::after {
    ...
    border: 0.3rem solid var(--fecha-color); /* Borde del círculo con el color de fecha. */
...
.credits a {
    color: var(--color); /* Aplica el color de texto definido a la variable. */
}
</style>